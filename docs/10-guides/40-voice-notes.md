...
`CompactBars` serves that case without decoding anything, and it is the reason monowave's missing AAC decoder is survivable.

## The strategy

The sender computes bars at record time and uploads roughly 64 bytes of metadata beside the audio.
The receiver draws directly from those bytes -- no decoder, no native code, no waiting, and no round trip to fetch the audio just to know what it looks like.

This is what WhatsApp, Telegram and Signal do, and it removes the entire decode path from the common case.
...
When you need those, you are in [drawing a waveform](./10-drawing.md) territory: `PeakWindow` and `WaveformViewport`, and a painter of your own.
